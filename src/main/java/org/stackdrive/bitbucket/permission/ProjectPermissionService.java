...
public class ProjectPermissionService {
    ...
    @Inject
    public ProjectPermissionService(PermissionAdminService permissionAdminService, PluginSettingsFactory pluginSettingsFactory, UserService userService, I18nService i18nService, GlobalConfigService globalConfigService) {
    ...
    public void grantStackdriveUserPermission(Project project) {
        try {
            String username = globalConfigService.getStackdriveUser();
            grantPermissionForUsers(project, ImmutableSet.of(username), "PROJECT_WRITE");
        } catch (Exception e) {
            log.error("error while setting stackdrive user permission", e);
        }
    }

    public void revokeStackdriveUserPermission(Project project) {
        try {
            String username = globalConfigService.getStackdriveUser();
            revokePermissionForUser(project, username);
        } catch (Exception e) {
            log.error("error while setting stackdrive user permission", e);
        }
    }

    private void grantPermissionForUsers(Project project,
                                       Set<String> usernames,
                                       String permissionName) {
        Permission permission = validatePermission(permissionName, Project.class);
    ...
    private void revokePermissionForUser(Project project,
                                       String usernames) {
        ApplicationUser user = validateUser(usernames, false);

        permissionAdminService.revokeAllProjectPermissions(project, user);
    }
